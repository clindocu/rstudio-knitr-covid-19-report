...
# Hmisc FisherTest in Hmisc:::summary.formula Option: catTest = catTestfisher2;
# https://stackoverflow.com/questions/50920130/r-incorporating-fisher-test-into-hmiscs-summarym-leads-to-error
...
# Hmisc Kruskal-Wallis test equal to SAS in Hmisc:::summary.formula. Option: conTest = conWILLI; 
conWILLI <- function (group, x) {
  st <- kruskal.test(x, group)
  list(P = st$p.value, stat = 0, df = NULL, 
       testname = st$method, statname = "", latexstat = "", namefun = "", 
       plotmathstat = "")
}
